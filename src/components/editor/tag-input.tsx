...
export function TagInput({
  ...
  return (
    <div>
      <div className="-mx-2 flex flex-wrap items-center gap-1.5 rounded-xl border border-transparent px-2 py-1.5 transition-colors hover:border-line focus-within:border-line-strong">
        {value.map((tag) => (
          <span
            key={tag}
            className="inline-flex items-center gap-1 rounded-full bg-sunken py-1 pr-1.5 pl-3 text-[13px] font-medium text-ink"
          >
            {tag}
            <button
              type="button"
              ...
              className="flex size-5 items-center justify-center rounded-full text-subtle transition-colors hover:bg-hover hover:text-ink"
            >
              <X className="size-3" />
            </button>
          </span>
