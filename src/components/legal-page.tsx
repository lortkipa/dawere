...
export function LegalPage({ title, updated, children }: { title: string; updated: string; children: ReactNode }) {
  return (
    <main className="mx-auto w-full max-w-[44rem] flex-1 px-5 pt-12 pb-20 sm:px-6 sm:pt-16">
      <p className="text-[13px] text-subtle">ბოლო განახლება: {updated}</p>
      <h1 className="mt-2 font-serif text-[2.1rem] leading-tight font-bold tracking-tight text-ink sm:text-[2.5rem]">
        {title}
      </h1>
